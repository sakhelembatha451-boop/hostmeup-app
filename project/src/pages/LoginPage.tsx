    ...
    <div className="min-h-screen flex bg-paper relative">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm animate-scale-in">
          <Link to="/" aria-label="HostMeUp home" className="font-display text-2xl font-bold text-ink tracking-tightest mb-12 inline-block">HostMeUp</Link>
          ...
          {/* Google sign-in */}
          <button onClick={handleGoogleSignIn} disabled={googleLoading || submitting}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)}
                  className="input-editorial w-full pl-8 pr-4 py-3 text-sm" placeholder="••••••••" />
              </div>
            </div>
          ...
          <p className="text-sm text-center text-ink-400 mt-8">
            Don't have an account? <Link to="/signup" aria-label="Create a new account" className="font-medium text-ink hover:text-accent transition-colors">Sign up</Link>
          </p>
        </div>
      </div>
      ...
      <div className="hidden lg:block w-2/5 relative overflow-hidden bg-paper-300">
        <img src="https://images.pexels.com/photos/17945059/pexels-photo-17945059.jpeg?auto=compress&cs=tinysrgb&h=1200&w=800"
          alt="Live music performance at an event venue" width={800} height={1200} className="absolute inset-0 w-full h-full object-cover" />
      </div>
